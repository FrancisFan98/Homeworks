...
let make initial =                  (*create a new record*)
  {
  current = initial;  
  undo_stack = [];
  redo_stack = [];
...
let undo document = 
  if document.undo_stack <> [] then(
    document.redo_stack <- document.current :: document.redo_stack;       (*update redo_stack*)
    document.current <- List.hd document.undo_stack;
    document.undo_stack <- List.tl document.undo_stack;                    (*update undo_stack*)
    true)
  else false                                                              (*return true is succeed else false*)
;;
...
let redo document = 
  if document.redo_stack <> [] then(
    document.undo_stack <- document.current :: document.undo_stack;                 (*update undo_stack*)   
    document.current <- List.hd document.redo_stack;
    document.redo_stack <- List.tl document.redo_stack;                              (*update redo_stack*)
    true)
  else false                                                                          (*return true is succeed else false*)
;;
