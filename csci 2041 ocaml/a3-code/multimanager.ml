...
let rec execute_command tokens =
  ...
  | "add" ->
     let newv = Sortedlist.insert global.curdoc.current tokens.(1) in   (*create a new list by using insert and set it to cur*)
     Document.set global.curdoc newv
  | "remove" ->
     let newv = Sortedlist.remove global.curdoc.current tokens.(1) in   (*create a new list by using remove and set it to cur*)
     Document.set global.curdoc newv
  | "save" ->
  ...
  | "load" ->
     let newv = Util.strlist_from_file tokens.(1) in                    (*load a new list by using strlist_from_file and set it to cur*)
     Document.set global.curdoc newv
  | "mergein" ->
     let load = Util.strlist_from_file tokens.(1) in                    (*create a new list by using strlist_from_file and merge and then set it to cur*)
     let newv = Sortedlist.merge load global.curdoc.current in
     Document.set global.curdoc newv
  | "undo" ->
     if Document.undo global.curdoc then ()                             (*check if undo_stack is empty, if so, undo the last operation else print error*)
     else printf "%s\n" "WARNING: undo list empty, no changes made"
  | "redo" ->
     if Document.redo global.curdoc then ()                             (*check if redo_stack is empty, if so, undo the last operation else print error*)
     else printf "%s\n" "WARNING: redo list empty, no changes made"

  (* --LIST MANAGEMENT COMMANDS-- *)
  | "lists" ->
     let newv = Doccol.string_of_doccol global in                       (*get the information of documents by string_of_doccol then print it*)
     printf "%s\n" newv
  | "edit" ->
     if Doccol.switch global tokens.(1) then ()                         (*edit by using switch call. check if switch is successful do nothing else print error*)
     else printf "ERROR: list '%s' does not exist\n" tokens.(1)         
  | "new" ->
     let newdoc = Document.make [] in                                   (*make a new document then add it to the docs and switch it to current document*)
     if Doccol.add global tokens.(1) newdoc then
     (
       if Doccol.switch global tokens.(1) then ()             
       else ()
     )
     else
       printf "ERROR: list '%s' already exists\n" tokens.(1)              (*if alreadi exist, print error message*)
  | "open" ->
     let newdoc = Document.make (Util.strlist_from_file tokens.(1)) in   (*create a new document with the data from the file as its current value*)
     if Doccol.add global tokens.(1) newdoc then(                         (*check if it already exist, if so switch to it else print error*)
        if Doccol.switch global tokens.(1) then ()                        (*no need for print error because it will always exist*)
        else ()
     )
     else(
        printf "ERROR: list '%s' already exists\n" tokens.(1)
  ...
  | "close" ->
     if Doccol.remove global tokens.(1) then ()                            (*check remove*)
     else 
      if global.curname = tokens.(1) then                                   (*if remove fail, check why and print the corresponding error*)
        printf "ERROR: cannot close the current list\n"
      else
        printf "ERROR: list '%s' does not exist\n" tokens.(1)
     
  | "merge" -> 
     let newlist = Sortedlist.merge (global.curdoc.current) ((List.assoc tokens.(1) global.docs).current) in    (*create new list by merge the current value and the value of target document*)
     Document.set global.curdoc newlist                                                                         (*set the new value*)
  ...
  | "mergeall" ->
     let newlist = Bulkops.mergeall global in                         (*create a new list by merge all the documents and set it to the new current value*)
     Document.set global.curdoc newlist
  (* Catch-all *)
  | _ ->
