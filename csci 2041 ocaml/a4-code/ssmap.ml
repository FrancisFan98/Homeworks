...
let rec getopt map key =
  ...
  | Empty -> None
  | Node(node) -> 
    let diff = String.compare key node.key in   (*check the difference of the two keys *)
...
let contains_key map str =
  getopt map str != None                         (*if getopt is not None, the value exist*)
;;
...
let rec fold func cur map =
  ...
  | Node(node) ->
    ...
    let ne = func n node.key node.value in                  (*do the calculations*)
    fold func ne node.right                                 (*go right and do the above recursively*)
;;
...
let to_string map =                                 (* verbose version: no use of iter *)
  ...
  let rec build map  = 
    ...
    | Node(node) -> 
      build node.left;
      Buffer.add_string buf "{" ;
      Buffer.add_string buf node.key;
      Buffer.add_string buf " -> ";
      Buffer.add_string buf node.value;
      Buffer.add_string buf "}, ";
      build node.right
    in
    build map;
    if String.length (Buffer.contents buf) < 2 then "[]"
	else "[" ^ (String.sub (Buffer.contents buf) 0 (String.length (Buffer.contents buf) -2) ) ^ "]"
;;  
