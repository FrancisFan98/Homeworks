...
module Make (ElMod : ELEM_SIG) = struct
  ...
  let getopt set elem =
    if ElTreemap.getopt set elem = None then None
    else Some elem
  ;;
  ...
  let iter func set =
    let nefunc s _ = func s in
    ElTreemap.iter nefunc set 
  ;;
  ...
  let fold func init set =
    let nefunc s elem _ = func s elem in
    ElTreemap.fold nefunc init set
  ;;
