...
let parse_tokens tokens =
  ...
  and prec2 toks =
    ...
    | Slash :: tail ->                        (* + is first *)
       ...
       (Div(lexpr,rexpr), rest)              (* return addition of these two *)
  ...
  and prec3 toks = 
    let (lexpr, rest) = prec2 toks in
    match rest with 
    |Plus  :: tail ->
      let (rexpr,rest) = prec3 tail in 
      (Add(lexpr, rexpr), rest)
    |_ -> (lexpr,rest)

  and prec4 toks = 
  let (lexpr, rest) = prec3 toks in 
  match rest with 
  |Minus :: tail -> 
    let (rexpr,rest) = prec4 tail in 
